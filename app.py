...
st.markdown("<p style='text-align: center; color: #64748b; margin-bottom: 30px;'>سیستەمی پێشکەوتوو بە هەردوو مۆدی خۆکار و دەستی</p>", unsafe_allow_html=True)

mode = st.radio("هەڵبژاردەی شێوازی کارکردن:", ["مۆدی خۆکار (Auto)", "مۆدی دەستی و ئارەزوومەندانە (Manual)"], horizontal=True)
st.markdown("---")
...
with col_settings:
    ...
    if mode == "مۆدی دەستی و ئارەزوومەندانە (Manual)":
        st.markdown("---")
    ...
    if st.button("🚀 دروستکردنی PDF", type="primary", use_container_width=True):
        if not raw_text.strip():
            ...
            with st.spinner("چاوەڕێ بە... ⏳"):
                ...
                st.success("✅ فایلەکە ئامادەیە.")
                st.download_button(label="📥 داگرتنی PDF", data=pdf_data, file_name=output_file, mime="application/pdf", use_container_width=True)

st.markdown("<hr><p style='text-align: center; color: #94a3b8;'>PDF Pro Engine</p>", unsafe_allow_html=True)
